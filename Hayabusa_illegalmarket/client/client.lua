    ...
    for _, v in pairs(Config.Items) do
        ...
        if item then
            options[#options+1] = {
                ...
                event = 'hayabusa_sellped:giveinput',
                args = {
...
RegisterNetEvent('hayabusa_sellped:giveinput', function (data)
    ...
    if input[1] ~= nil then
        if input[2] ~= nil then
            TriggerServerEvent('hayabusa_sellped:sellitem',data.item, data.price, input[1], input[2])
        else
